...
file = open("/home/pi/Diplomarbeit/RC-Car/data/data.txt",'a+')  #Open the datafile in append-mode if it exists, create if it it doesn't exist
...
imu = MPU9250.MPU9250(bus, address)     #set MPU9250 as the seleced IMU
...
port="/dev/ttyAMA0" #Define GPS Device
ser=serial.Serial(port, baudrate=9600) #initialize UART
...
while 1:    #main loop
    ...
    for i in range(10): #run the sensorfusion algorythm 10x faster than the sensor gets read
        ...
        dt = newTime - currTime     #calculate the diffrence between the last and the new time
        currTime = newTime          #write the new time into currTime for the next cycle
        sensorfusion.updateRollPitchYaw(imu.AccelVals[0], imu.AccelVals[1], imu.AccelVals[2], imu.GyroVals[0], imu.GyroVals[1], imu.GyroVals[2], imu.MagVals[0], imu.MagVals[1], imu.MagVals[2], dt)    #call the sensorfusion algorythom
    ...
    if print_count == 10:   #every 10 cycles write the data to the SD Card
        ...
        newdata=ser.readline()      #Get new GPS data
        newmsg=pynmea2.parse(newdata) #parse new data
        lat=newmsg.latitude         #save Lati
        lng=newmsg.longitude        #save Long
        ...
        if pitch < 0:                       #check if x-Offset should be substracted
            ax = imu.AccelVals[0] - xoffs   #substract x-Offset
        elif pitch > 0:                     #check if x-Offset should be added
            ax = imu.AccelVals[0] + xoffs   #add x-Offset

        if flipa < 0:                       #check if y-Offset should be substracted
            ay = imu.AccelVals[1] - yoffs   #substract y-Offset
        elif flipa > 0:                     #check if y-Offset should be added
            ay = imu.AccelVals[1] + yoffs   #add y-Offset
        
        if a*b < 0:                         #check if z-Offset should be substracted
            az = imu.AccelVals[2] - zoffs   #substract z-Offset
        elif a*b > 0:                       #check if z-Offset should be added
            az = imu.AccelVals[2] + zoffs   #add z-Offset

        gps = str(lat) + ", " + str(lng) #Define Lat and Long as gps
        ...
        print("Lat/Long: " + str(gps))      #print GPS readings
        ...
        file.write(str(gps))                #write GPS readings
        file.write("\n")                    #write newline
